...
def ReadMITGCM(
    ...
    da_Kwx2 = np.concatenate(
        (da_Kwx[:, :, :, -1:], da_Kwx[:, :, :, :-1]), axis=3
    )
    da_Kwy2 = np.concatenate(
        (da_Kwy[:, :, :, -1:], da_Kwy[:, :, :, :-1]), axis=3
    )
    da_Kwz2 = np.concatenate(
        (da_Kwz[:, :, :, -1:], da_Kwz[:, :, :, :-1]), axis=3
    )
    da_Eta2 = np.concatenate((da_Eta[:, :, -1:], da_Eta[:, :, :-1]), axis=2)
    da_lon2 = np.concatenate((da_lon[-1:], da_lon[:-1]), axis=0)
    density2 = np.concatenate(
        (density[:, :, :, -1:], density[:, :, :, :-1]), axis=3
    )
    ...
    if save_arrays:
        ...
        outputs_tr_DelT_filename = (
            "../../../INPUT_OUTPUT_ARRAYS/SinglePoint_OutputsDelTTr.npy"
        )
        outputs_val_DelT_filename = (
            "../../../INPUT_OUTPUT_ARRAYS/SinglePoint_OutputsDelTVal.npy"
        )
        outputs_te_DelT_filename = (
            "../../../INPUT_OUTPUT_ARRAYS/SinglePoint_OutputsDelTTe.npy"
        )
        outputs_tr_Temp_filename = (
            "../../../INPUT_OUTPUT_ARRAYS/SinglePoint_OutputsTempTr.npy"
        )
        outputs_val_Temp_filename = (
            "../../../INPUT_OUTPUT_ARRAYS/SinglePoint_OutputsTempVal.npy"
        )
        outputs_te_Temp_filename = (
            "../../../INPUT_OUTPUT_ARRAYS/SinglePoint_OutputsTempTe.npy"
        )
