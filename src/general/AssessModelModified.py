...
def plot_scatter(
    ...
    if not xlabel:
        xlabel = "True temperature change (" + "\xb0" + "C)"
        xlabel_filename = "truth"
    else:
        xlabel_filename = xlabel
    if not ylabel:
        ylabel = "Predicted temperature change (" + "\xb0" + "C)"
    ...
    if exp_cor == True:
        ...
        ax1.annotate(
            "Mean Squared Error: " + str(np.format_float_scientific(mse, 2)),
            (0.22, 0.87),
    ...
    plt.savefig(
        ...
        bbox_inches="tight",
        pad_inches=0.1,
    )  #  Leave as png, far too large filesize if eps!
    plt.close()
